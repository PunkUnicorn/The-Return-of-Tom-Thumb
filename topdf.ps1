...
Get-Content -Path "Prose - Chapter One*.md" | python spellchecker.py | ConvertFrom-Json | %{ $_.Results } | %{ Add-AppveyorMessage -Message "Chapter One - Spelling - $($_.Word)" -Details "$($_.Hint)" -Category "Error" }
...
Get-Content -Path "Prose - Chapter One*.md" | python thesaurunocerous.py | ConvertFrom-Json | %{ $_.Results } | %{ Add-AppveyorMessage -Message "Chapter One - Thesaurunocerous - $($_.Word)" -Details "$($_.Hint)" -Category "$($_.Status)" }
