...
Get-Content -Path "Prose - Chapter One*.md" | python spellchecker.py | %{(,$d=$_.Split("!")) | Add-AppveyorTest -Name "Spelling" -Framework NUnit -Filename "$d[0]" -Outcome "$d[1]" -ErrorMessage "$d[2]" }
