...
Function WordAnalysis-Chapter($chapterName) {
	...
	$chapterWordCount = $chapterContent | python3 wordcounter.py | ConvertFrom-Csv 
...
Function Thesaurunocerous-Chapter($chapterName, $wordsFilename) {
  ...
  if ($env:WANTTHES -eq "1")
  {
		...
		$chapterTheasurus = $chapter | python3 thesaurunocerous.py | ConvertFrom-Json | %{ $_.Results }
		#$chapterTheasurus | fl
...
Write-Output "... made The-Return-of-Tom-Thumb.html..."
 ...
 cat gTTS_croconossorus_word_input.txt | python3 .\googleTextToSpeech.py -o A_Croconossorus_Tale.mp3 -d A_Croconossorus_Tale.mp3.log 
 ...
 cat test1.txt | python3 .\googleTextToSpeech.py -o testymctestface.mp3 -d testymctestface.mp3.log #-l fr-FR
